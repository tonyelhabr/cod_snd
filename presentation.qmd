---
title: "Streaking and Clutching in Round-Based First Person Shooter (FPS) Esports"
author: "Tony ElHabr"
format:
  revealjs:
    incremental: false
...
o_round_win_text_layer <- function(..., .emphasize = 'low') {
  if(.emphasize == 'low') {
    op <- `<`
    .color <- blackish_background
  } else {
...
p_o_round_win_prop
save_heatmap(p_o_round_win_prop)
```
...
p_o_round_win_prop_over_expected <- o_win_prop |> 
  ggplot() +
  common_cod_heatmap_layers() +
  pre_cumu_heatmap_tile(.fill = 'diff_win_round_prop') +
...
save_heatmap(p_o_round_win_prop_over_expected)
```
...
p_o_series_win_prop <- o_win_prop |> 
  ggplot() +
  common_cod_heatmap_layers() +
  pre_cumu_heatmap_tile(.fill = 'win_series_prop') +
...
p_o_series_win_prop
save_heatmap(p_o_series_win_prop)
```
...
p_o_series_win_prop_over_expected <- o_win_prop |> 
  ggplot() +
  common_cod_heatmap_layers() +
  pre_cumu_heatmap_tile(.fill = 'diff_win_series_prop') +
...
save_heatmap(p_o_series_win_prop_over_expected)
```
...
::: notes
-   There should be evidence of 6-0's and 6-5's being more frequent than expected given 50-50% chances.
:::
...
b2b_round_win_props_tile_layer <- function(..., .round, .palette, .is_new = TRUE) {
  ...
  list(
    ...
    init,
    geom_tile(
      data = b2b_round_win_props |> filter(n_rounds == .round),
      color = blackish_background,
      height = 0.8,
      show.legend = FALSE,
      aes(
...
b2b_round_win_prop_text_layer <- function(..., .emphasize) {
  ...
  list(
    ...,
    geom_text(
      data = b2b_round_win_props |> filter(`op`(rnk, 1)),
      color = .color,
      size = 11 / .pt,
      fontface = 'bold',
      aes(
...
b2b_round_win_props_arws <- tibble(
  x = c(9.3, 9.4, 9.75, 10),
...
p_b2b_round_win_props <- b2b_round_win_props |> 
  ggplot() +
  b2b_round_win_props_tile_layer(.round = 7, .palette = 'pink', .is_new = FALSE) +
  b2b_round_win_props_tile_layer(.round = 8, .palette = 'light-blue') +
  b2b_round_win_props_tile_layer(.round = 9, .palette = 'light-green') +
  b2b_round_win_props_tile_layer(.round = 10, .palette = 'amber') +
  b2b_round_win_props_tile_layer(.round = 11, .palette = 'deep-orange') +
  ...
  map(
    b2b_round_win_props_arws |> group_split(rn), 
    function(.x) {
      geom_curve(
        data = data.frame(),
        color = 'white',
...
save_heatmap(p_b2b_round_win_props)
...
![](b2b_round_win_props.png){fig-align="center"}
