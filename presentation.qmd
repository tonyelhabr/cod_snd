---
title: "They're on Fire: Team Momentum in Round-Based First Person Shooter Esports"
author: 'Tony ElHabr'
format:
  revealjs:
    slide-number: true
...
```{r}
#| label: altnerative-to-p=0.5-method-1
...
cod_theoretical_series_lengths <- cod_ps |> 
  mutate(
...
cod_theoretical_constant_p <- cod_theoretical_series_lengths |> 
  mutate(
    diff = theoretical_series_length - cod_rounds_per_series$actual
  ) |> 
  slice_min(abs(diff), n = 1) |> 
  pull(p)
cod_theoretical_constant_p
```

```{r}
#| label: altnerative-to-p=0.5-method-2
...
n10 <- pluck_cod_n(10)
n11 <- pluck_cod_n(11)
pm <- pmin(n6, n7, n8, n9, n10, n11)
n6adj <- round(10 * n6 / pm)
n7adj <- round(10 * n7 / pm)
n8adj <- round(10 * n8 / pm)
n9adj <- round(10 * n9 / pm)
n10adj <- round(10 * n10 / pm)
n11adj <- round(10 * n11 / pm)

maximize_cod_series_p <- function(p) {
  prob_of_series_lasting_n_games(n = 6, p = p, max_round = 11)^n6adj *
    prob_of_series_lasting_n_games(n = 7, p = p, max_round = 11)^n7adj * 
    prob_of_series_lasting_n_games(n = 8, p = p, max_round = 11)^n8adj * 
    prob_of_series_lasting_n_games(n = 9, p = p, max_round = 11)^n9adj *
    prob_of_series_lasting_n_games(n = 10, p = p, max_round = 11)^n10adj * 
    prob_of_series_lasting_n_games(n = 11, p = p, max_round = 11)^n11adj
}

cod_theoretical_constant_p <- cod_ps |> 
  mutate(
    theoretical_series_length = map_dbl(p, maximize_cod_series_p)
  )

cod_theoretical_constant_p <- cod_theoretical_constant_p |> 
  slice_max(theoretical_series_length, n = 1) |> 
  pull(p)
cod_theoretical_constant_p

m <- matrix(c(cod_streaks_res$series$n_actual, cod_streaks_res$series$n_expected), ncol = 2)
dimnames(m) <- list(n_rounds = as.character(6:11), kind = c('actual', 'expected'))
ch <- chisq.test(m)
# ch$expected
# chance_numbers <- tibble(
#   league = rep(c('mlb', 'nba', 'nhl'), each = 4),
#   i = rep(4:7, 3),
#   a = c(
#     c(0.1894, 0.215, 0.215, 0.3895),
#     c(0.1324, 0.25, 0.3382, 0.2794),
#     c(0.2532, 0.2405, 0.3038, 0.2025)
#   ),
#   e = rep(c(0.125, 0.25, 0.3125, 0.3125), 3)
# )
# chance_numbers |> filter(league == 'mlb') |> select(a, e) |> chisq.test()
```
