...
There's a clear bias towards shorter series than expected.
...
cod_round_win_prop_after_x_wins_and_rounds <- tibble(win_streak = 2L:4L) |>
  mutate(
    data = map(win_streak, summarize_cod_win_prop_after_x_wins_and_rounds)
  ) |>
  unnest(data)
save_qs(cod_round_win_prop_after_x_wins_and_rounds)
```
