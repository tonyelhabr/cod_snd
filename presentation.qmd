...
## Is there evidence for the "hot hand" ([COD SnD]{.cod})?
...
summarize_cod_win_prop_after_x_wins <- function(k) {
  ...
  cod_round_win_prop_after_x_wins <- cod_round_streaks_after_x |> 
...
```{r}
#| label: cod_round_win_prop_after_x_wins-eval
#| eval: true
...
summarize_cod_win_prop_after_x_wins_y_rounds <- function(k) {
  ...
  cod_round_streaks_after_x <- cod_round_streaks |> 
    drop_na(!!!col_syms) |> 
    count(n_rounds, round, win_round, won_prior_round1, !!!col_syms, sort = TRUE)
  ...
  cod_round_win_prop_after_x_wins |> 
    ...
    ungroup() |> 
    filter(win_round) |> 
    select(n_rounds, round, n, total, prop) |> 
    arrange(n_rounds, round) |> 
    inner_join(
      probs_nx_k,
      by = c('n_rounds', 'round')
    ) |> 
    nest(data = -c(n_rounds, round)) |> 
    mutate(
      gt_p_value = map_dbl(
        data, 
        ~binom.test(.x$n, .x$total, .x$expected_poststreak_p, alternative = 'greater') |> 
...
## Is there evidence for the "hot hand" ([Valorant]{.valorant})?

```{r}
#| label: val_round_win_prop_after_x_wins-save
#| eval: false
val_round_streaks <- val_rounds |> 
  group_by(series_id, team, is_offense) |> 
  mutate(
    won_prior_round1 = lag(win_round, n = 1, default = NA),
    won_prior_round2 = lag(win_round, n = 2, default = NA),
    won_prior_round3 = lag(win_round, n = 3, default = NA),
    won_prior_round4 = lag(win_round, n = 4, default = NA),
    won_prior_round5 = lag(win_round, n = 5, default = NA),
    won_prior_round6 = lag(win_round, n = 6, default = NA),
    won_prior_round7 = lag(win_round, n = 7, default = NA),
    won_prior_round8 = lag(win_round, n = 8, default = NA),
    won_prior_round9 = lag(win_round, n = 9, default = NA)
  ) |> 
  ungroup() |>
  select(
    series_id,
...
summarize_val_win_prop_after_x_wins_first12r <- function(k) {
  stopifnot((k >= 3) & (k <= 9))
  probs_nx_k <- tibble(
    cumu_w = 4L:11L,
    n_rounds = 12L
  ...
  cols <- sprintf('won_prior_round%d', 2:k)
  col_syms <- syms(cols)
  val_round_streaks_after_x <- val_round_streaks |> 
    filter(round < 13) |> 
    drop_na(!!!col_syms) |> 
    count(cumu_w, win_round, won_prior_round1, !!!col_syms, sort = TRUE)
  
  val_round_win_prop_after_x_wins <- val_round_streaks_after_x |> 
    filter(won_prior_round1, !!!col_syms)
  
  val_round_win_prop_after_x_wins |> 
    group_by(cumu_w) |> 
    mutate(
      total = sum(n),
      prop = n / total
...
summarize_val_win_prop_after_x_wins_second12r <- function(k) {
  stopifnot((k >= 3) & (k <= 9))
  probs_nx_k <- crossing(
    cumu_w = 0L:13L,
    n_rounds = 13L:24L
  ...
  val_round_streaks_after_x <- val_round_streaks |> 
    filter(round > 12) |> 
    drop_na(!!!col_syms) |> 
    count(n_rounds, cumu_w, win_round, won_prior_round1, !!!col_syms, sort = TRUE)
  
  val_round_win_prop_after_x_wins <- val_round_streaks_after_x |> 
    filter(won_prior_round1, !!!col_syms)
  
  val_round_win_prop_after_x_wins |> 
    group_by(n_rounds, cumu_w) |> 
    mutate(
      total = sum(n),
      prop = n / total
    ) |> 
    ungroup() |> 
    filter(win_round) |> 
    select(n_rounds, cumu_w, n, total, prop) |> 
    arrange(n_rounds, cumu_w) |> 
    inner_join(
      probs_nx_k,
      by = c('n_rounds', 'cumu_w')
    ) |> 
    nest(data = -c(n_rounds, cumu_w)) |> 
    mutate(
      gt_p_value = map_dbl(
        data, 
        ~binom.test(.x$n, .x$total, .x$expected_poststreak_p, alternative = 'greater') |> 
...
val_round_win_prop_after_x_wins_first12r <- tibble(win_streak = 3L:9L) |> 
  mutate(
    data = map(win_streak, summarize_val_win_prop_after_x_wins_first12r)
  ) |> 
  unnest(data)
save_qs(val_round_win_prop_after_x_wins_first12r)

val_round_win_prop_after_x_wins_second12r <- tibble(win_streak = 3L:9L) |> 
  mutate(
    data = map(win_streak, summarize_val_win_prop_after_x_wins_second12r)
  ) |> 
  unnest(data)
save_qs(val_round_win_prop_after_x_wins_second12r)
```

```{r}
#| label: val_round_win_prop_after_x_wins
#| eval: false
load_qs('val_round_win_prop_after_x_wins_first12r')
load_qs('val_round_win_prop_after_x_wins_second12r')
val_round_win_prop_after_x_wins_first12r |> 
  filter(n > 30) |> 
  mutate(
    gt_signif = gt_p_value <= 0.05
  ) |> 
  ggplot() +
  aes(x = win_streak, y = cumu_w) +
  geom_tile(
    aes(fill = gt_signif)
  ) +
  geom_text(
...
val_round_win_prop_after_x_wins_second12r |> 
  filter(n > 30) |> 
  mutate(
    gt_signif = gt_p_value <= 0.05
  ) |> 
...
```{r}
#| label: cod_round_win_prop_after_xw-eval
#| eval: false
load_qs('val_round_win_prop_after_x_wins_first12r')
val_round_win_prop_after_x_wins_first12r |> 
  transmute(
    `Win Streak` = win_streak,
    `# of Offensive Wins in First 12 Rounds` = cumu_w,
    `N` = scales::comma(total),
    across(c(prop, expected_poststreak_p), ~scales::percent(.x, accuracy = 0.1)),
    across(
      ends_with('p_value'), 
...
::: fragment
-   Given the nature of how economy is used in the game, perhaps this is not suprising at all.
:::
