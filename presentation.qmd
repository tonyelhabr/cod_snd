...
heatmap_tile <- function(..., .xmin, .xmax, .ymin, .ymax, .fill) {
  list(
    ...,
    geom_rect(
...
pre_cumu_heatmap_tile <- function(..., .fill) {
  heatmap_tile(
    .xmin = 'pre_cumu_w',
    .ymin = 'pre_cumu_l',
    .fill = .fill,
...
p_o_round_win_prop <- o_win_prop |> 
  ...
  pre_cumu_heatmap_tile(.fill = 'win_round_prop') +
...
b2b_round_win_props |> 
  ggplot() +
  pre_cumu_heatmap_tile(.fill = 'prop')
...
b2b_round_win_props |> 
  ...
  group_by(n_rounds) |> 
  mutate(
    intra_prop = (n - mean(n)) / sd(n)
  ) |> 
  ggplot() +
  aes(x = n_rounds, y = round) +
  geom_tile(aes(fill = intra_prop))
