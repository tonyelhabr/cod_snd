...
![](val_o_win_r1_vs_post_round_prop.png){fig-align="center"}
...
summarize_streaks <- function(rounds, game = c('cod', 'val')) {
  rounds <- val_rounds
  game = 'val'
  .max_round <- ifelse(game == 'cod', 11, 25)
  
  sudden_death_rounds <- rounds |> 
    filter(max_round > .max_round)
  
  rounds1 <- rounds
  ## should only happen in val
  if(nrow(sudden_death_rounds) > 0) {
    
    sudden_death_series_ids <- sudden_death_rounds |> 
      distinct(series_id)
    
    imputed_suddent_death_rounds <- sudden_death_rounds |> 
      mutate(
        round = .max_round
      ) |> 
      select(series_id, round, is_offense) |> 
      inner_join(
        rounds,
        by = c('series_id', 'round', 'is_offense')
      )
    
    rounds <- bind_rows(
      rounds |> 
        filter(max_round <= .max_round),
      imputed_suddent_death_rounds
    )
  }
  ...
  list(
    rounds = series_streak_prop,
    series = series_outcome_prop
  )
}

cod_streaks_res <- cod_rounds |> summarize_streaks(game = 'cod')
val_streaks_res <- val_rounds |> summarize_streaks(game = 'val')
cod_long_series_outcome_prop <- cod_streaks_res$series |>
  select(
    record,
    n = n_actual,
    `3` = prop_actual,
