---
title: '"Turned on!": Momentum in Round-Based First Person Shooter Esports'
author: 'Tony ElHabr'
format:
  revealjs:
    slide-number: true
...
-   Valorant Champions Tour (VCT) matches (qualifiers and majors), through November 11, 2021
-   5v5
-   First to 13 round wins
-   One team plays offense in 12 consecutive rounds before swapping sides
-   Must win by 2 cod_rounds
:::
...
val_o_win_prop_const <- val_rounds |> 
  ...
  filter(win_round) |> 
  pull(prop)
val_o_win_pct <- scales::percent(val_o_win_prop_const, accuracy = 1)
```
...
## What are your chances of winning a round, given the series state in [COD SnD]{.cod}?
...
# round_and_series_win_prop <- cod_rounds |> 
...
heatmap_text_layer <- function(..., .op, .df, .label, .num, .color, .threshold, .labeller) {
...
save_for_slide(p_cod_o_round_win_prop)
```
...
![](cod_o_round_win_prop.png){fig-align="center"}
...
save_for_slide(p_cod_o_round_win_prop_over_expected)
```
...
![](cod_o_round_win_prop_over_expected.png){fig-align="center"}

## What are your chances of winning a round, given the series state in [Valorant]{.valorant}?
...
save_for_slide(p_val_o_round_win_prop)
```

::: notes
-   Big difference compared to COD. Lower/higher percentages in the corners are probably more indicative of gaps in team skill.
-   1-0, 1-1, 3-0, 3-1 seem to b e the biggest outliers.
:::

![](val_o_round_win_prop.png){fig-align="center"}

## What are your chances of winning the series, given the series state in [COD SnD]{.cod}?
...
save_for_slide(p_cod_o_series_win_prop)
```
...
![](cod_o_series_win_prop.png){fig-align="center"}
...
save_for_slide(p_cod_o_series_win_prop_over_expected)
```
...
![](cod_o_series_win_prop_over_expected.png){fig-align="center"}

## What are your chances of winning the series, given the series state in [Valorant]{.valorant}?
...
save_for_slide(p_val_o_series_win_prop)
```

![](val_o_series_win_prop.png){fig-align="center"}
...
save_for_slide(p_val_o_series_win_prop_over_expected)
```

![](val_o_series_win_prop_over_expected.png){fig-align="center"}
...
calculate_cod_o_round_win_prop <- function(df) {
  df |> 
...
summarize_cod_o_round_win_prop <- function(.round) {
  
  specified_round <- cod_rounds |> 
    filter(round == .round) |> 
    calculate_cod_o_round_win_prop()
  
  post_rounds <- cod_rounds |> 
    filter(round > .round) |> 
    calculate_cod_o_round_win_prop()
...
plot_cod_round_vs_post_round_slope_chart <- function(.round) {
  o_win_rx_prop <- summarize_cod_o_round_win_prop(.round)
...
cod_res_r1 <- plot_cod_round_vs_post_round_slope_chart(1)
...
save_for_slide(p_cod_o_win_r1_vs_post_round_prop)
```

![](cod_o_win_r1_vs_post_round_prop.png){fig-align="center"}
...
cod_actual_series_streaks <- cod_rounds |> 
  filter(win_series) |> 
...
cod_series_streaks <- inner_join(
  cod_actual_series_streaks |> 
...
cod_series_streak_props <- cod_series_streaks |> 
...
cod_series_outcome_prop <- full_join(
  cod_actual_series_streaks |> 
    group_by(record) |> 
    summarize(
      across(n, sum)
...
cod_long_series_outcome_prop <- cod_series_outcome_prop |>
  select(
    record,
    n = n_actual,
    `3` = prop_actual,
...
cod_max_diff_series_outcome_prop <- cod_long_series_outcome_prop |> 
  filter(outcome_idx == 1) |> 
  pull(value) |> 
  abs() |> 
  max()

p_cod_series_outcome_prop_over_expected <- cod_long_series_outcome_prop |> 
  ggplot() +
  aes(x = record, y = outcome_idx) +
  geom_tile(
    data = cod_long_series_outcome_prop |> filter(outcome_idx > 1),
    show.legend = FALSE,
    # height = 0.8,
    aes(fill = value)
  ) +
  ...
  scale_fill_gradient2(
    ...
    midpoint = 0,
    limits = c(
      -cod_max_diff_series_outcome_prop, 
      +cod_max_diff_series_outcome_prop
    )
  ) +
  geom_text(
    data = cod_long_series_outcome_prop |> filter(outcome_idx == 3),
    color = blackish_background,
    size = 11 / .pt,
    aes(label = sprintf('%.1f%%\n(%s)', 100 * value, scales::comma(n)))
  ) +
  geom_text(
    data = cod_long_series_outcome_prop |> filter(outcome_idx == 2),
    color = blackish_background,
    size = 11 / .pt,
    aes(label = sprintf('%.1f%%', 100 * value))
  ) +
  geom_text(
    data = cod_long_series_outcome_prop |> filter(outcome_idx == 1) |> filter(abs(value) != max(abs(value))),
    color = blackish_background,
    size = 11 / .pt,
    aes(label = sprintf('%+.1f%%', 100 * value))
  ) +
...
save_for_slide(p_cod_series_outcome_prop_over_expected)
```

![](cod_series_outcome_prop_over_expected.png){fig-align="center"}
...
p_cod_b2b_round_win_prop <- cod_b2b_round_win_prop |> 
  ...
  labs(
    ...
    x = 'Round',
    y = '# of cod_rounds in Complete Series'
  ) + 
  geom_text(
    data = tibble(
...
save_for_slide(p_cod_b2b_round_win_prop)
...
![](cod_b2b_round_win_prop.png){fig-align="center"}
...
::: notes
-   Note this is sort of biased by the fact that teams end up winning series in 6 cod_rounds more often than expected.
:::
