...
observe that \(P_E(r)\) is notably larger for \(p_{2}\) when
...
\begin{equation}\protect\hypertarget{eq:pwkr}{}{
  \hat{p}^{kr,\text{win}} = \hat{p}(\text{win round | k-round winning streak, series lasting r rounds})=\{
    \begin{array}{cl}
...
\(\hat{p}^{kr,loss} = \hat{p}(\text{lose round | k-round losing streak, series lasting r rounds})\)
can be formulated symmetrically.

Miller et al. (2018) provide evidence for negative recency suggesting
that, given a deterministic i.i.d. Bernoulli probability (Equation
\ref{eq:pki},

\begin{equation}\protect\hypertarget{eq:pkrd}{}{
E[\hat{P}^{kr,\text{win}} - \hat{P}^{kr,\text{loss}}] = 0
}\label{eq:pkrd}\end{equation}

is not true due to streak-selection bias.\footnote{Note that these
  probabilities are separate from those in Section \ref{sec:analysis-1}.}
I don't attempt to measure this expectation like Miller et al. (2018),
who perform their analysis longitudinally per study participant;
nonetheless, I apply their formulation for applying the ``downward
bias'' on post-streak success to come up with estimates
\(\hat{P}_{kr,\text{win}}^{\text{adj.}}\), specifying \(r\) as the
number of trials.
...
\(\hat{P}_{kr,\text{win}}\) for \(k = 3\) winning streaks, for both
teams that empirically won and lost the series. (Some combinations of
\(r\) and \(\text{team loses series}\) are irrelevant, such as the case
when \(r \in [7, 8, 9]\), so these combinations are excluded.)
