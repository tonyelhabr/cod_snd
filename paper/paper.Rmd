...
cod_round_win_prop_after_k_wins |> 
  ...
  gt::cols_label(
    ...
    n_rounds = 'Series lasts $i$ rounds',
    total = 'N',
    prop = 'Obs.',
    prior_p = 'Naive',
    expected_poststreak_p = 'Discounted',
    gt_p_value = 'Naive',
    gt_p_value_adj = 'Discounted'
  ) |> 
  gt::tab_spanner(
    label = '%s',
    columns = c(prop, prior_p, expected_poststreak_p)
...
```
$\hat{P}_{lki} = \hat{P}(\text{lose round | k-round losing streak, series lasting i rounds})$ can be formulated symmetrically.

Miller et al. [-@miller2018] provide evidence for negative recency suggesting that
...
```
given a deterministic i.i.d. Bernoulli probability (Equation \ref{eq:pki}.[^13]

[^13]: Note that these probabilities are separate from those in Section \ref{sec:analysis-1}.

Given that people typically perceive streaks as beginning after the third success (or failure) at minimum [@carlson2007], I focus on round win percentages in rounds immediately following streaks of three wins.[^14] Table \ref{tbl:cod-round-win-prop-after-k-wins} shows $\hat{P}_{wki}$ for $k = 3$ winning streaks, for both teams that empirically won and lost the series. (Some combinations of $i$ and $\text{team loses series}$ are irrelevant, such as the case when $i \in [7, 8, 9]$, so these combinations are excluded.)
