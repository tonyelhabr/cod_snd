...
Chance [-@chance2020] re-examines the constant probability notion in Major League Baseball's World Series (1923--2018), the National Basketball Association's Finals (1951--2018), and the National Hockey League's Stanley Cup (1939--2018). Chance finds strong evidence against the null hypothesis of $p = 0.5$ in the MLB and NHL championship series when applying Mosteller's first and second methods.[\^8] [\^8]: Chance goes on to outline a conditional probability framework (likelihood of winning a game given the series state) which can exactly explain the distribution of the number of games played.
...
Studying both player streaks and team streaks in basketball, in both observational and controlled settings. Gilovich et al. [-@gilovich1985] do not find evidence for the hot hand phenomenon. However, Miller and Sanjurjo [-@miller2018] refuted the conclusions of Gilovich et al., proving a framework for quantifying streak selection bias, which effectively works in the manner posited by the gambler's fallacy[^9]. Specifically, Miller and Sanjurjo say that a "bias exists in a common measure of the conditional dependence of present outcomes on streaks of past outcomes in sequential data" implying that, under i.i.d. conditions, "the proportion of successes among the outcomes that immediately follow a streak of consecutive successes is expected to be strictly less than the underlying (conditional) probability of success". When applying streak selection bias to Giloveih et al.'s data, Miller and Sanjurjo come to the opposite conclusions as Giloveih et al.

[^9]: Streak selection bias, "in conjunction with a quasi-Bayesian model of decision making under sample size neglect... provides a novel structural candidate explanationfor the persistence of gambler's fallacy beliefs", per Miller and Sanjurjo [-@miller2018].
...
The general formula for the probability $P_E(r)$ that a best-of-$s$ series ($s = 11$ for CoD SnD) lasts $r$ rounds given constant probability $p$ of one team[^11] winning each round is

[^11]: If $p > 0.5$, then one might say that this team is the better team (known in hindsight).
...
```
The chi-square goodness of fit statistic can be used to test the constant found probability hypothesis for $0 \leq p \leq 1$.
...
Without accounting for the streak selection bias described by Miller and Sanjurjo [-@miller2018], the round win probability after a streak of $k$ round wins for a best-of-$s$ series ($s = 11$ for CoD SnD), accounting for the series lasting $r$ rounds known in hindsight, is
...
\end{equation}
```
The "hindsight" round loss probability after a streak of $k$ round losses for a series lasting $r$ rounds, $p^{l|kr}$, can be formulated symmetrically.[^12]

[^12]: Note that these probabilities are separate from the constant round probabilities in Sections \ref{sec:method-rounds-played} and \ref{sec:results-rounds-played}.

However, as shown by Miller and Sanjurjo, we should expect the observed proportion of rounds wins after a streak $k$ rounds wins for a series lasting $r$ rounds, $\hat{P}^{w|kr}$, to be strictly less than $p^{w|kr}$.[^13]

[^13]: See Appendix A of Miller and Sanjurjo [-@miller2018] for the proof.

> Theorem 1: *Let* $\mathbf{X} = \{X_i\}^{n}_{i=1}, n \geq 3$*, be a sequence of independent Bernoulli trials, each with probability of success* $0 < p < 1$*. Let* $\hat{P}_k(\mathbf{X})$ *be the proportion of successes on the subset of trials* $I_k(\mathbf{X})$ *that immediately follow* $k$ consecutive successes, that is, $\hat{P}_k(\mathbf{X}) := \sum_{i \in I_k(\mathbf{X})} X_i | I_k(\mathbf{X}) |. \hat{P}_k$ *is a biased estimate of* $\mathbb{P}(X_t = 1 | \prod_{j=t-k}^{t-1} X_j = 1) \equiv p$ *for all* $k$ *such that* $1 \leq k \leq n - 2$*. In particular,* $E[\hat{P}_k(\mathbf{X}) | I_k(\mathbf{X}) \neq \emptyset] < p.$

Miller and Sanjurjo [-@miller2018] note that there does not exists a closed form representation for $E[\hat{P}_k(\mathbf{X}) | I_k(\mathbf{X}) \neq \emptyset]$ ($\hat{P}^{w|kr}$ for CoD SnD) for $k > 1$. Figure \ref{fig:npk} shows this.[^14]

[^14]: I've adapted the code from Vafa [-@vafa2017].

![The expected value](images/npk_replicated.png)

The expected value of the proportion of successes on trials that immediately follow k consecutive successes, Pˆk(X), as a function of the total number of trials n,for different values of k and probabilities of success p,

# Results

First, I investigate the constant probability assumption and the distribution of rounds played in a series. Chance's [-@chance2020] work is closely related to mine, and, in fact, provides a guide for this investigation.

## Distribution of rounds played {#sec:results-rounds-played}

Using Equation \ref{eq:chi-squ}, $\chi^2 = 16.0$ (p-value of 0.0068) for a naive constant round win probability $p_0 = 0.5$. Thus, I can comfortably reject the constant probability null hypothesis for $p_0 = 0.5$, even at a confidence level of $\alpha = 0.01$. Afterwards, I investigate post-streak win rates, building on my learnings from the constant probability assumption analysis. I agree with Miller and Sanjurjo's [-@miller2018] framework for streak selection bias, accounting for it in when analyzing momentum.

```{r}
#| label: cod_series_outcome_prop
.max_round <- 11
.cutoff <- 6
prob_of_series_lasting_r_rounds <- function(r, p = 0.5, s = .max_round) {
  m <- as.integer((s + 1) / 2)
  (factorial(r - 1) / (factorial(m - 1) * factorial(r - s))) * (p^m * (1 - p)^(r - m) + p^(r - m) * (1 - p)^m)
}
...
summarize_cod_streaks <- function(p = 0.5) {
  ...
  expected_round_streaks <- tibble(
    ...
    mutate(
      series_prop = map_dbl(n_rounds, ~prob_of_series_lasting_r_rounds(.x, max_round = .max_round, p = !!p)),
...
Table \ref{tbl:cod-prob-series-lasting-r-rounds} shows the expected probabilities for $s = 11$ under the naive assumption that $p_0 = 0.5$ for a given round, along with the observed proportions $P_O(r)$ in CoD SnD.
...
theoretical_cod_series_length <- function(p) {
  6 * prob_of_series_lasting_r_rounds(n = 6, p = p, max_round = 11) +
    7 * prob_of_series_lasting_r_rounds(n = 7, p = p, max_round = 11) +
    8 * prob_of_series_lasting_r_rounds(n = 8, p = p, max_round = 11) +
    9 * prob_of_series_lasting_r_rounds(n = 9, p = p, max_round = 11) +
    10 * prob_of_series_lasting_r_rounds(n = 10, p = p, max_round = 11) +
    11 * prob_of_series_lasting_r_rounds(n = 11, p = p, max_round = 11)
}
...
maximize_cod_series_p <- function(p) {
  prob_of_series_lasting_r_rounds(n = 6, p = p, max_round = 11)^cod_n6adj *
    prob_of_series_lasting_r_rounds(n = 7, p = p, max_round = 11)^cod_n7adj *
    prob_of_series_lasting_r_rounds(n = 8, p = p, max_round = 11)^cod_n8adj *
    prob_of_series_lasting_r_rounds(n = 9, p = p, max_round = 11)^cod_n9adj *
    prob_of_series_lasting_r_rounds(n = 10, p = p, max_round = 11)^cod_n10adj *
    prob_of_series_lasting_r_rounds(n = 11, p = p, max_round = 11)^cod_n11adj
}
...
cod_streaks_other_res |>
  ...
  inner_join(
    cod_streaks_naive_res$series |>
      transmute(n_rounds, `$P_O(r)$` = scales::percent(prop_actual, accuracy = 0.1)),
    by = 'n_rounds'
  ) |>
  arrange(n_rounds) |>
  rename(`$r$` = n_rounds) |>
...
Table \ref{tbl:mosteller-methods-results} shows the alternate values for the constant round win probability $p$ that I find when applying the three methods suggested by Mosteller [-@mosteller1952]. Each is approximately or equal to 0.575, and, when applying Equation \ref{eq:chi-squ}, each results in a $\chi^2$ value for which I cannot reject the constant probability null hypothesis.
...
\end{longtable}
```
Table \ref{tbl:expected-series-lengths-alternative-ps} shows the new $P_E(r)$ when re-applying Equation \ref{eq:series-length} for the maximum likelihood estimate $p_2 = 0.575$.[^15] I observe that $P_E(r)$ is notably larger for $p_{2}$ when $r \in [6, 7]$, more closely matching $P_O(r)$. $p_{2}$ is also closer to $P_O(r)$ for $r \in [9, 10]$, although not for $r \in [8, 11]$.

[^15]: The method of moments and minimum $\chi^2$ estimates for $p$ are omitted simply because the results would be nearly identical to those for the maximum likelihood estimate of $p$ (since they are all $\approx 0.575$).
...
postprocess_round_streaks <- function(k, probs_nx_k, ...) {
  ...
  suppressMessages(
    cod_round_win_prop_after_z_with_probs <- cod_round_win_prop_after_z |> 
      ...
      ungroup() |> 
      filter(win_round) |> 
      select(n_rounds, ..., win_series, n, total, prop) |> 
      arrange(n_rounds, ...) |> 
      inner_join(
        probs_nx_k
      )
  ...
  cod_round_win_prop_after_z_with_probs |> 
    nest(data = -c(n_rounds, win_series, ...)) |> 
    mutate(
      gt_p_value = map_dbl(
        data, 
        ~binom.test(.x$n, .x$total, .x$prior_p, alternative = 'greater') |> 
          broom::tidy() |>
          pull(p.value)
      ),
      lt_p_value = map_dbl(
        data, 
        ~binom.test(.x$n, .x$total, .x$prior_p, alternative = 'less') |> 
          broom::tidy() |>
          pull(p.value)
      ),
      gt_p_value_adj = map_dbl(
        data, 
        ~binom.test(.x$n, .x$total, .x$expected_poststreak_p, alternative = 'greater') |> 
          broom::tidy() |>
          pull(p.value)
      ),
      lt_p_value_adj = map_dbl(
        data, 
      ...
      gt_is_signif_adj = gt_p_value_adj <= 0.05,
      lt_is_signif_adj = lt_p_value_adj <= 0.05
    ) |> 
    unnest(data)
}
...
summarize_cod_win_prop_after_k_wins <- function(k) {
  probs_nx_k <- crossing(
    ...
    select(-is_valid) |> 
    mutate(
      prior_p = (ifelse(win_series, 6L, n_rounds - 6L) - k) / (n_rounds - k),
...
cod_round_win_prop_after_k_wins <- tibble(win_streak = 2L:4L) |>
  mutate(
...
cod_round_win_prop_after_k_wins |> 
  mutate(
    ...
    unadj_lab = sprintf('%.0f%%', round(100 * prior_p), gt_p_value),
    adj_lab = sprintf('%.0f%%', round(100 * expected_poststreak_p), gt_p_value_adj)
  ) |>
  filter(win_streak == 3, !win_series) |> 
  arrange(win_streak, n_rounds, win_series) |> 
  transmute(
    # win_streak,
    n_rounds,
    # across(win_series, ~ifelse(.x, 'win', 'lose')),
    total,
    prop,
    unadj_lab,
    adj_lab
  ...
  gt::tab_spanner(
    label = 'x',
    columns = c(prior_p, gt_p_value)
  ) |> 
  gt::tab_spanner(
    label = 'y',
    columns = c(expected_poststreak_p, gt_p_value_adj)
  ) |> 
  gt::as_latex()
```

Given that people typically perceive streaks as beginning after the third success (or failure) at minimum [@carlson2007], I focus on round win percentages in rounds immediately following streaks of three wins.[^16] Table \ref{tbl:cod-round-win-prop-after-k-wins} shows $\hat{P}^{kr,\text{win}}_O$ and $\hat{P}^{kr,\text{loss}}_O$ for $k = 3$ streaks respectively.

[^16]: Three happens to also be a reasonable number for series that lasts at maximum 11 rounds.
