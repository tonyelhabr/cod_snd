...
cod_round_win_prop_after_k_wins |> 
  ...
  mutate(
    ...
    prop_lab = sprintf('%.1f', 100 * prop),
    notional_lab = sprintf('%.1f (%s)', 100 * notional_prop, notional_p_value_adj),
    ms_lab = sprintf('%.1f (%s)', 100 * ms_prop, ms_p_value_adj)
  ) |>
...
cod_round_win_prop_after_k_wins_i_rounds |> 
  ...
  arrange(n_rounds, round, win_series) |> 
  gt::gt() |> 
  gt::cols_label(
    n_rounds = 'r',
    round = 'i',
...
cod_round_arrangements_mle |> 
  ...
  mutate(
    ...
    across(prop_p_value, list(adj = ~p.adjust(.x, method = 'BY'))),
    across(
      matches('p_value'), 
      ~case_when(
        .x <= 0.01 ~ '<=0.01', 
    ...
    across(ws, ~str_replace_all(.x, '-', ' ') |> str_replace_all(c('1' = '+', '0' = '-'))),
    actual_lab = sprintf('%.2f', 100 * prop_actual),
    expected_lab = sprintf('%.2f (%s)', 100 * prop_expected, prop_p_value)
  ) |> 
  select(
    n_rounds, 
    ws,
...
Another idea would be to account for the quality of the teams. Even if we still cannot prove that the hot hand effect exists when doing so, a null finding would be insightful in and of itself.
