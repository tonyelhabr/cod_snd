...
The hindsight Bernoulli round loss probability after a streak of $k$ round wins, $p^{l|kr}_0$, can be formulated symmetrically. We might consider these as a reference when considering the
...
MS note that there does not exists a closed form representation for $E[\hat{P}_k(\mathbf{X}) | I_k(\mathbf{X}) \neq \emptyset]$ for $k > 1$. ($\hat{P}_k(\mathbf{X})$ is the analogue for $\hat{P}^{w|kr}_{MS}$ in my notation for CoD SnD.) Figure 1 shows how the expected value varies as a function of the total number of trials $n$, given $p$ and $k > 1$.[^13]
...
Consequently, a statistical test of the difference in $\hat{P}^{w|kr}$ and $\hat{P}^{l|kr}$, as performed by GVT and MS to evaluate their respective hypothesis regarding $E[\hat{P}_k(\mathbf{X}) | I_k(\mathbf{X}) \neq \emptyset]$ (Theorem 1), is not appropriate.
...
~~When considering momentum in consecutive rounds wins, I use the descriptor "hindsight" because I account for the length of the series (~~$r$ rounds).
...
\end{equation}
```
A binomial test for the observed proportion of round wins following streaks of $k$ round wins in a series lasting $r$ rounds, $P^{w|kr}$, assuming $p_0^{w|kr}$ as the probability of success, can be used to verify that $p_0^{w|kr}$ is reasonable as a reference.
...
summarize_cod_win_prop_after_k_wins <- function(k) {
  probs_nx_k <- crossing(
    ...
    select(-is_valid) |> 
    mutate(
      naive_p = ifelse(win_series, 6L / n_rounds, (n_rounds - 6L) / n_rounds),
      prior_p = (ifelse(win_series, 6L, n_rounds - 6L) - k) / (n_rounds - k),
      across(c(naive_p, prior_p), ~ifelse(win_round, .x, 1 - .x)),
...
Given that people typically perceive streaks as beginning after the third success (or failure) at minimum [@carlson2007], I focus on streaks of three round wins.[^15] Table \ref{tbl:cod-round-win-prop-after-k-wins} shows $P^{w,kr}$ and $P^{l,kr}$ for round win rate and loss rate following $k = 3$ round win streaks respectively.
