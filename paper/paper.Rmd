...
Afterwards, I investigate post-streak win rates, building on my learnings from the constant probability assumption analysis. I agree with Miller and Sanjurjo's [-@miller2018] framework for streak selection bias, accounting for it in when analyzing momentum.

## Distribution of rounds played {#sec:analysis-1}

The general formula for the probability $P_E(r)$ that a best-of-$s$ series lasts $r$ rounds given constant probability $p$ of one team[^10] winning each round is

[^10]: If $p > 0.5$, then I might say that this team is the better team (known omnisciently).
...
\cmidrule(lr){3-4}
Series lasts $r$ rounds & $P_E(r)$ & $P_O(r)$ & $N_O(r)$ \\ 
\midrule
6 & $3.1\%$ & $4.7\%$ & 40 \\ 
7 & $9.4\%$ & $11.9\%$ & 101 \\ 
8 & $16.4\%$ & $16.5\%$ & 141 \\ 
...
cod_streaks_other_res |>
  ...
  arrange(n_rounds) |>
  rename(`Series lasts $r$ rounds` = n_rounds) |>
  knitr::kable()
```
...
\end{longtable}
```
Table \ref{tbl:expected-series-lengths-alternative-ps} shows the new $P_E(r)$ when re-applying Equation \ref{eq:series-length} for the maximum likelihood estimate $p_2 = 0.575$.[^11] I observe that $P_E(r)$ is notably larger for $p_{2}$ when $r \in [6, 7]$, more closely matching $P_O(r)$. $p_{2}$ is also closer to $P_O(r)$ for $r \in [9, 10]$, although not for $r \in [8, 11]$.

[^11]: The method of moments and minimum $\chi^2$ estimates for $p$ are omitted simply because the results would be nearly identical to those for the maximum likelihood estimate of $p$ (since they are all $\approx 0.575$.
...
\toprule()
Series lasts \(r\) rounds & \(p_0(r)\) = 0.5 & \(p_2(r)\) = 0.575 & \(P_O(r)\) \\
\midrule()
\endhead
6 & 3.1\% & 4.2\% & 4.7\% \\
7 & 9.4\% & 11.2\% & 11.9\% \\
...
cod_round_win_prop_after_k_wins |> 
  ...
  gt::fmt_percent(
    decimals = 0,
    columns = c(prop, prior_p, expected_poststreak_p)
  ) |> 
  gt::cols_label(
    # win_streak = 'Streak',
    win_series = 'Win Series?',
    n_rounds = 'Series lasts r rounds',
    total = 'N',
    prop = 'O',
    prior_p = 'a',
    expected_poststreak_p = 'b',
    gt_p_value = 'a',
    gt_p_value_adj = 'b'
  ) |> 
  gt::tab_spanner(
  ...
  gt::tab_spanner(
    label = 'y',
    columns = c(gt_p_value, gt_p_value_adj)
  ) |> 
  gt::tab_footnote(
    footnote = 'Binom',
    locations = gt::cells_column_labels(columns = c(gt_p_value))
  ) |> 
  gt::as_latex()
```

Despite the implication that one can model team round win probability in CoD SnD as a constant, anecdotally many believe in momentum i.e. streaks not due to random chance. I now evaluate win frequency in rounds following a streak of length $k$. I account for the length of the series ($r$ rounds), modeling the expected round win probability after a streak of length $k$ as

```{=tex}
\begin{equation}\protect\hypertarget{eq:pwkr}{}{
  \hat{p}^{kr,\text{win}}_E = \hat{p}(\text{win round | k-round winning streak, series lasting r rounds})=\{
    \begin{array}{cl}
...
\end{equation}
```
$\hat{p}^{kr,loss}_E = \hat{p}(\text{lose round | k-round losing streak, series lasting r rounds})$ can be formulated symmetrically.

While one might think that $E[\hat{p}^{kr,\text{win}} - \hat{p}^{kr,\text{loss}}] = 0$, Miller and Sanjurjo [-@miller2018] suggest that, given a deterministic i.i.d. Bernoulli probability (Equation \ref{pwkr}, this is not true due to streak-selection bias.[^12][^13]

[^12]: Note that these probabilities are separate from those in Section \ref{sec:analysis-1}.

[^13]: Three happens to also be a reasonable number for series that lasts at maximum 11 rounds.

I apply their formulation for "downward bias" to post-streak success to come up with estimates $\hat{P}_{kr,\text{win}}^{\text{adj.}}$, specifying $r$ as the number of trials.

Given that people typically perceive streaks as beginning after the third success (or failure) at minimum [@carlson2007], I focus on round win percentages in rounds immediately following streaks of three wins.[^14] Table \ref{tbl:cod-round-win-prop-after-k-wins} shows $\hat{P}_{kr,\text{win}}$ for $k = 3$ winning streaks, for both teams that empirically won and lost the series. (Some combinations of $r$ and $\text{team loses series}$ are irrelevant, such as the case when $r \in [7, 8, 9]$, so these combinations are excluded.)

[^14]: Three happens to also be a reasonable number for series that lasts at maximum 11 rounds.

```{=tex}
\begin{longtable}{rrrrrrrr}
\toprule
&  &  & \multicolumn{3}{c}{$\hat{p}^{kr,\text{win}}$} & \multicolumn{2}{c}{$p$-value} \\
\cmidrule(lr){4-6} \cmidrule(lr){7-8}
Series lasts $r$ rounds & Win Series? & $N^{kr,\text{win}}_O$ & $O$ & $E$ & $\text{adj.}$ & $E$ & $\text{adj.}$ \\ 
\midrule
7 & win & 209 & $75\%$ & $75\%$ & $61\%$ & 0.58 & <=0.01 \\ 
8 & win & 209 & $62\%$ & $60\%$ & $45\%$ & 0.28 & <=0.01 \\ 
9 & win & 193 & $52\%$ & $50\%$ & $35\%$ & 0.33 & <=0.01 \\ 
10 & lose & 60 & $13\%$ & $14\%$ & $10\%$ & 0.64 & 0.26 \\ 
10 & win & 151 & $44\%$ & $43\%$ & $29\%$ & 0.45 & <=0.01 \\ 
11 & lose & 129 & $24\%$ & $25\%$ & $16\%$ & 0.63 & <=0.01 \\ 
11 & win & 150 & $40\%$ & $38\%$ & $25\%$ & 0.29 & <=0.01 \\ 
\bottomrule
\end{longtable}
```
