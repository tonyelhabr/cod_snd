...
While one might think that $E[\hat{p}^{kr,\text{win}} - \hat{p}^{kr,\text{loss}}] = 0$, Miller et al. [-@miller2018] suggest that, given a deterministic i.i.d. Bernoulli probability (Equation \ref{pwkr}, this is not true due to streak-selection bias.[^13][^14]
...
I don't attempt to measure this expectation like Miller et al. [-@miller2018], who perform their analysis longitudinally per study participant; nonetheless, I apply their formulation for applying the "downward bias" on post-streak success to come up with estimates $\hat{P}_{kr,\text{win}}^{\text{adj.}}$, specifying $r$ as the number of trials.

Given that people typically perceive streaks as beginning after the third success (or failure) at minimum [@carlson2007], I focus on round win percentages in rounds immediately following streaks of three wins.[^15] Table \ref{tbl:cod-round-win-prop-after-k-wins} shows $\hat{P}_{kr,\text{win}}$ for $k = 3$ winning streaks, for both teams that empirically won and lost the series. (Some combinations of $r$ and $\text{team loses series}$ are irrelevant, such as the case when $r \in [7, 8, 9]$, so these combinations are excluded.)
